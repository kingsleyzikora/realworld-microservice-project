...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t kingsleyzikora/frontendservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker kingsleyzikora/frontendservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push kingsleyzikora/frontendservice:latest "
                    }
                }
            }
        }
        ...
        //             sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.147.65.219 "docker run -t zaproxy/zap-weekly zap-baseline.py -t http://18.222.133.185:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#zikora-kingsley', //update and provide your channel name
